...
import sys,getopt
from testproject.helper import sha256_hmac
import ast
...
def get_key_for_sensor(sensor):
    try:
        text_file = open("./testproject/helper/integrity_secrets.txt", "r")
...
def message_handler(client,msg,topic):
    if topic==topic_control: #got control message
        ...
        if key is not None:
            ...
            try:
                msg = msg.split(".")
                integrity_hash = sha256_hmac.generate_hash(key, msg[1])
                print("Integrity hash: " + integrity_hash)
                if msg[0] == integrity_hash:
                    update_status(client, msg[1])
                else:
                    print("Integrity check failed")
            except Exception as e:
                print("Error on integrity check")
        else:
            print("Error on integrity check")
...
def publish_status(client):
    ...
    if client.sensor_status_old!=client.sensor_status or pubflag:
        ...
        key = get_key_for_sensor(sensor)
        if key is not None:
            integrity_hash = sha256_hmac.generate_hash(key, client.sensor_status)
            message = integrity_hash + "." + client.sensor_status
            client.publish(sensor_status_topic, message, options["qos"], True) #here is the qos set
            print("publish on: " + sensor_status_topic + " message: " + message)
            client.last_pub_time=time.time()
            client.sensor_status_old=client.sensor_status
        else:
            print("Error on creating integrity hash")
