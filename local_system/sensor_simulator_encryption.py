...
import sys,getopt
from testproject.helper import aes_encryption
import ast
...
options["port"]=1883 #1883 or 8883 for tls
options["verbose"]=False
options["username"]=""
options["password"]=""
options["cname"]=""
options["sensor_type"]="light"
options["topic_base"]="sensorstopic"
...
def get_key_for_sensor(sensor):
    try:
        text_file = open("./testproject/helper/encryption_secrets.txt", "r")
...
def message_handler(client,msg,topic):
    if topic==topic_control: #got control message
        ...
        sensor = client._client_id.decode("utf-8")
        key = get_key_for_sensor(sensor)
        if key is not None:
            print("Key: " + key + "for " + sensor)
            try:
                decrypted_payload = aes_encryption.decrypt_message(msg, key, padding_character)
                print("Decrypted payload: " + decrypted_payload)
                update_status(client,decrypted_payload)
            except Exception as e:
                print("Error on payload decryption")
        else:
            print("Error on payload decryption")
...
def publish_status(client):
    ...
    if client.sensor_status_old!=client.sensor_status or pubflag:
        #encrypt before publishing again
        sensor = client._client_id.decode("utf-8")
        key = get_key_for_sensor(sensor)
        if key is not None:
            encrypted_payload = aes_encryption.encrypt_message(client.sensor_status, key, padding_character)
            print("Encrypted payload for publish: " + encrypted_payload.decode("utf-8"))
            client.publish(sensor_status_topic, encrypted_payload, options["qos"], True)  # here is the qos set
            print("publish on ", sensor_status_topic, " message: ", encrypted_payload)
            client.last_pub_time = time.time()
            client.sensor_status_old = client.sensor_status
        else:
            print("Error on payload encryption")
    ...
    while not connflag:
        ...
        try:
            ...
            if res==0:
                ...
                logging.info("connection failed ",res)
                badcount +=1
