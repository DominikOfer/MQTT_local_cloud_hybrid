...
options=dict()
brokers=["rasperrypi", "10.0.0.100"]
options["broker"]=brokers[0]
options["port"]=8883 #1883 or 8883 for tls
...
def publish_status(client):
    ...
    if client.sensor_status_old!=client.sensor_status or pubflag:
        client.publish(sensor_status_topic,client.sensor_status,options["qos"],True) #here is the qos set
...
def Initialise_clients(cname):
    ...
    client.tls_set(ca_certs=certs_location + 'ca.crt', certfile=certs_location + 'sensor.crt', keyfile=certs_location + 'sensor.key', cert_reqs=ssl.CERT_REQUIRED)
